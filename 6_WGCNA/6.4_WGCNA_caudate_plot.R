...
bw_label2color <- read.table("/data/MyProgram/Final_diabrain/7.scripts/6_WGCNA/Brain_Caudate_bw_label2color.txt", header = T)
...
pdf(paste0("./Module_Trait_", mytissue, ".pdf"), width = 4, height = 9)
par(mar = c(6, 10, 3, 3))
labeledHeatmap(Matrix = bwmoduleTraitCor[, "DIABETES", drop = FALSE], xLabels = "DIABETES", xLabelsAngle = 0, xLabelsAdj = 0.5, 
               ...
               cex.text = 1.0, cex.lab = 1.2, zlim = c(-1,1))
dev.off()
...
pdf("./GSvsMM_interest_mod.pdf", width = 12, height = 12)
par(mfrow = c(2,2))
...
pdf("./GS_across_module.pdf", width = 15, height = 9)
plotModuleSignificance(geneTraitSignificance[[1]], bwnetColors)
dev.off()
