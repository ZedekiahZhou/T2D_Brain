...
# Version:  3.0
# Date:     Mar 5, 2019
# New function: bootstrap for 1000 times in 20:10 sample size
#		collect all the info include up down and different p value
# ====================================================================================================
...
degenes <- array(NA, dim = c(length(fname), 9, PTIMES), 
                 dimnames = list(sub("Brain_", "", tname), 
                                 c("total1", "up1", "down1", "total05", "up05", "down05", "total01", "up01", "down01"),  
                                 c(1:PTIMES)))
...
DEfun <- function(select_sam) {
    ...
    tres1 <- results(tdds, alpha=0.1)
    tres05 <- results(tdds, alpha=0.05)
    tres01 <- results(tdds, alpha=0.01)
    
    #Summary--------------------------------------------------------------------
    res <- numeric(0)
    res[1] <- sum(tres1$padj < 0.1, na.rm = T)
    res[2] <- sum((tres1$padj < 0.1)&(tres1$log2FoldChange>0), na.rm = T)
    res[3] <- sum((tres1$padj < 0.1)&(tres1$log2FoldChange<0), na.rm = T)
    res[4] <- sum(tres05$padj < 0.05, na.rm = T)
    res[5] <- sum((tres05$padj < 0.05)&(tres05$log2FoldChange>0), na.rm = T)
    res[6] <- sum((tres05$padj < 0.05)&(tres05$log2FoldChange<0), na.rm = T)
    res[7] <- sum(tres01$padj < 0.01, na.rm = T)
    res[8] <- sum((tres01$padj < 0.01)&(tres01$log2FoldChange>0), na.rm = T)
    res[9] <- sum((tres01$padj < 0.01)&(tres01$log2FoldChange<0), na.rm = T)
...
#for (i in 1:length(fname)) {
	...
	sample_size <- 10
        ...
        for (j in 1:maxn) {
	    degenes[i, , j] <- DEfun(all_comb[, j])
	}

        write.table(degenes[i, , ], paste0("/home/zhouzhe/Final_diabrain/DESeq2data/", tname[i], "_moreinfo_s10"), 
                    quote = F, row.names = T, col.names = F, na = "NA")
...
save.image(paste0("/home/zhouzhe/Final_diabrain/DESeq2data/more_info", posnum[1],"_", posnum[2], ".RData"))
