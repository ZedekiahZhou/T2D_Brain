...
tmp <- load("/data/MyProgram/Final_diabrain/7.scripts/7_GWAS_REGION/script/hippocampus/region_v1.0.RData")
...
DEgene <- read.table("../../../2.results/Cov_optimal/result05/res/Brain_Hippocampus.res", header = T, stringsAsFactors = F)
togene <- DEgene$Gene
...
write.table(mod_gmx, file = "./for_GSEA/module_hippocampus_v1.0.gmx", quote = F, row.names = F, sep = "\t", na = "")
...
write.table(tmp, file = "./hippocampus/mod_GSEA.txt", quote = F, sep = "\t", row.names = F)
...
# write.table(GOdf, file = "./hippocampus/GO.tab", quote = F, sep = "\t", row.names = F)
# write.table(KEGGdf, file = "./hippocampus/KEGG.tab", quote = F, sep = "\t", row.names = F)
# 
# #save(GOdf, KEGGdf, mod_summary, mod2gene, modgene, DEgene, bwnet, file = "../hippocampus_covar_WGCNA.RData")
