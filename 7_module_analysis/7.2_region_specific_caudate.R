...
pdf("../../../4.plots/WGCNA_Caudate/region_specificity.pdf", height = 12, width = 9)
...
labeledHeatmap(Matrix = celltype_plot, xLabels = rep("", dim(celltype_plot)[2]), #xLabelsPosition = "top", #xSymbols = colnames(celltype_plot), 
               ...
               cex.text = 1.5, cex.lab = 1.5, zlim = c(0, max_celltype))
par(mar = c(9, 35, 0, 3))
labeledHeatmap(Matrix = region_plot, xLabels = colnames(region_plot), xSymbols = colnames(region_plot),
               ...
               colorLabels = FALSE, colors = blueWhiteRed(50)[26:50],  setStdMargins = F, 
               cex.text = 1.5, cex.lab = 1.5, zlim = c(0, max_region))
dev.off()
...
save(modgene_symbol, region_pSI, celltype_pSI, r0001, c0001, valuable_subMod001, valuable_subMod01, file = "./caudate/region_v1.0.RData")
